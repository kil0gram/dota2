...
namespace dotadata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var steamaccount = SteamAccount.GetSteamAccount(Common.steamaccountUrl, Common.API, SteamID);
            
            //Get list of latest heroes with names parsed/cleaned
            List<Heroes.Hero> heros = Heroes.GetHeroes(Common.herosUrl, Common.API);
            
            //get latest 100 matches with brief details (no hero items/abilities/build info)
            List<Match> latest100matches = MatchHistory.GetMatchHistory(Common.matchhistoryUrl, Common.API, heros);

            //Get match details for match id 1277955116
            MatchDetails.MatchDetailsResult matchdetails = MatchDetails.GetMatchDetail(Common.matchdetailsUrl, Common.API, 1277955116, heros);
